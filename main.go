...
func main() {
	...
	query, err := api.MakeMessage(api.Query{DeviceName: deviceName})
	must(err)

	_, err = api.ParseMessage(query, api.Query{})
	must(err)
	...
	i, err := api.ParseMessage(buffer[:length], api.Response{})
	must(err)

	response := i.(api.Response)
